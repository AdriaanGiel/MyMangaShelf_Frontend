...
export default function ReadingSpot({spot, readingSpotAction, rating}) {
    ...
    return <Card width="w-full" addon="p-2 py-4">
        <View className="flex flex-row gap-2 h-18  w-full">
            <Pressable className="flex flex-row" onPress={readingSpotAction}> 
            <View
                className="h-14 w-14 bg-dark-greon flex items-center justify-center rounded-md">
                <Image
            ...
            <View className="w-[60%] px-2">
                <Text
                    className={`truncate text-2xl ${getColors.getThemeString('text-dark-standard-text', appTheme)}`}>{ spot?.name ?? "Spot name" }</Text>
                <MapIcon color={getColors.getHexColor('dark-greon')}></MapIcon>

            </View>
            </Pressable>
            <View
                className="h-14 w-14 flex flex-row gap-5 justify-center items-center rounded-md">

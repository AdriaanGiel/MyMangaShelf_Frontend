...
export default function StandardMediaCard({item, folders}) {
    ...
    const SmallModal = () => {
        if (open) {
            return (
                <View
                    className="absolute z-30 w-full flex justify-center items-center h-full bottom-0 right-0">
                    <View className="bg-black w-full h-full absolute opacity-50"></View>

                    <ScrollView
                        nestedScrollEnabled={true}
                        className="bg-black max-h-40 w-full flex ">
                        {
    ...
    return (
        <Pressable
            ...
            <View className="px-2 pb-2">
                <Text
                    className={`text-xl font-bold ${getColors.getThemeString('text-dark-standard-text', appTheme)}`}>{media.title}</Text>
                <View className="flex flex-row justify-between">
                    <Text
                        className={`font-bold ${getColors.getThemeString('text-dark-neutral-text', appTheme)}`}>Volumes: {media.volumes}</Text>
                    <Text
                        className={`font-bold ${getColors.getThemeString('text-dark-neutral-text', appTheme)}`}>{media.published_year}</Text>

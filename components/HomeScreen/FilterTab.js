                ...
                <Pressable  onPress={() => switchTabs(0)}> 
                    <Text
                        className={(
                            ...
                            : `${getColors.getThemeString('text-dark-neutral-text', appTheme)} text-2xl`}>All</Text>
                </Pressable>
                                ...
                                <Pressable key={item.id} onPress={() => switchTabs(item.id)}>
                                    <Text
                                        className={(
                                            item.id == activeTab)
                                            ? `${getColors.getThemeString("text-dark-greon", appTheme)} text-2xl`
                                            : `${getColors.getThemeString('text-dark-neutral-text', appTheme)} text-2xl`}>{item.name}</Text>
                                </Pressable>
                            );
                        })
                }

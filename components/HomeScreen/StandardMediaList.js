...
export default function StandardMediaList({ items, folders, paginate }) {
  ...
  return (
    <View className="mb-48 flex gap-2">
      <CustomFlatList
        className=""
        contentContainerClassName="flex flex-row flex-wrap gap-5 justify-center"
        data={media}
        renderItem={({ item }) => (
          <StandardMediaCard folders={folders} key={item.id} item={item} />
        )}></CustomFlatList>

      <View className="flex  gap-1 h-10 flex-row w-full">
        <Pressable
          onPress={() => getMoreMedia("prev")}
          className="w-1/2 bg-dark-greon pt-2 flex items-center">
          <Text>Previous</Text>
        </Pressable>
        <Pressable
          onPress={() => getMoreMedia("next")}
          className="w-1/2 bg-dark-greon pt-2 flex items-center">
          <Text>Next</Text>
        </Pressable>
      </View>

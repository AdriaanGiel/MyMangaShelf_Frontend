...
export default function UserMediaCard({item, index, removeFunc}) {
    ...
    const SmallModal = () => {
        if (remove) {
            return (
                <View
                    className="absolute w-full z-30 flex justify-center items-center h-full bottom-0 right-0">
                    ...
                    <Pressable
                        onPress={toggleSmallModal}
                        className="mt-20 p-2 bg-purple-400 rounded-md">
    ...
    return (
        <Pressable
            ...
            <View className="px-2 pb-2">
                <Text className={`text-xl font-bold ${getColors.getThemeString('text-dark-standard-text',appTheme)}`}>{media.title}</Text>
                <View className="flex flex-row justify-between">
                    <Text className={`font-bold ${getColors.getThemeString('text-dark-neutral-text',appTheme)}`}>Volumes: { media.volumes }</Text>
                    <Text className={`font-bold ${getColors.getThemeString('text-dark-neutral-text',appTheme)}`}>{ media.published_year}</Text>

                </View>
            </View>
            { isLoading ? (<View className="w-full h-full flex justify-center items-center"><ActivityIndicator color={`${getColors.getHexColor('dark-greon')}`}></ActivityIndicator></View>) : (<SmallModal/>) }

...
export default function AddSource({syncLibrary}) {
    ...
    return (
        <View
            ...
            <View>
                <AutocompleteDropdown inputHeight={50} inputContainerStyle={{
                        ...
                        color: "#FFFF"
                    }} renderItem={(item, text) => <Text className="text-white p-5">{item.title}</Text>} className="bg-manga-400" clearOnFocus={false} closeOnBlur={true} closeOnSubmit={false} initialValue={source
                        ?.id}
                    // or just '2'
                    onSelectItem={changeSource} dataSet={providers}/>
            </View>
            <Pressable onPress={sync} className={`bg-manga-400 rounded-xl p-4 ${getColors.getThemeString('bg-dark-greon',appTheme)}`}>
                <Text className={`text-xl text-center font-semibold ${getColors.getThemeString('text-dark-standard-text',appTheme)}`}>SYNC LIBRARY</Text>
            </Pressable>
        </View>
    );
}

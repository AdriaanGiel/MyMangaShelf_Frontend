    ...
    <View className={`flex rounded-2xl h-24 ${getColors.getThemeString('bg-dark-card',appTheme)}`}>
      <View className="flex flex-row justify-between w-full h-full">
        ...
        <View className="px-3 w-8/12 justify-center">
          <Text className={`font-semibold text-xl ${getColors.getThemeString('text-dark-standard-text',appTheme)}`}>{ item?.name || "NAME" }</Text>
          <Text className={`${getColors.getThemeString('text-dark-standard-text',appTheme)}`}>{ item?.website || "url" }</Text>
        </View>

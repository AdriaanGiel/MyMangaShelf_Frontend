...
export default function MySpotScreen() {
    ...
    const SpotsItems = () => {
        ...
        if(!myLocations.length){
            return <Text className={ `${appTheme ? "text-dark-standard-text" : "text-light-standard-text"} text-3xl` }>You have not added any spots yet</Text>
        }

        return myLocations.map((spot, index) => <MyReadingSpot key={spot.name} spot={spot}></MyReadingSpot>)
    ...
    return <IsLoading state={!myLocations}>
        <ScrollView  className={`flex flex-1 gap-5  py-2 ${getColors.getThemeString('bg-dark-background', appTheme)}`}>

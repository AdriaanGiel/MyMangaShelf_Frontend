import {Image, KeyboardAvoidingView, Pressable, SafeAreaViewBase, ScrollView, StyleSheet, Text, TextInput, View} from "react-native";
...
export default function AddSpotScreen() {
    ...
    const getCurrentLocation = async () => {
        ...
        const currentLocation = await Location.getCurrentPositionAsync(
            {accuracy: Location.Accuracy.Lowest, mayShowUserSettingsDialog: true}
        );
...
const submitNewReadingSpot = async () => {
    ...
    await Storage.addData("user_spots", [
        ...items,
        newSpot
    ])
    ...
    return <IsLoading state={location === undefined} >
        <View className={`flex h-[100%]  py-4 gap-4  px-2 ${getColors.getThemeString("bg-dark-background",appTheme)}`}>
 ...
 <View className="w-full flex items-center">
        <GeneralCard custom={`${appTheme ? "bg-white": "bg-gray-900"} w-[95%] gap-4 min-h-20 py-4 px-2 items-center`}> 

            <Text className={`${appTheme ? 'text-light-standard-text': 'text-white'} text-3xl`}>Add a new reading spot</Text> 

            <TextInput placeholderTextColor={appTheme ? getColors.getHexColor('text-light-standard-text') : getColors.getHexColor('color-white') } placeholder="New reading spot" className={`${appTheme ? 'text-light-standard-text': 'color-white bg-black'} border w-[90%] rounded-md`}
            onChangeText={(value) => handleChange("name", value)}
            value={formData.name}/>  

            <TextInput
                placeholderTextColor={appTheme ? getColors.getHexColor('text-light-standard-text') : getColors.getHexColor('color-white') }
            ...
            <Pressable onPress={submitNewReadingSpot} className={`${appTheme ? 'text-light-standard-text': 'text-white'} rounded bg-dark-greon p-2 flex items-center w-[33%]`}> 
                <Text>Submit</Text>  
            </Pressable>

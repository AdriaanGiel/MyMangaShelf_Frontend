...
export default function ReadingSpotScreen() {
    ...
    const LoadingReadingSpots = () => {
        if (!locations) {
            return <GeneralCard
                custom={`${appTheme
                    ? "bg-white"
                    : "bg-dark-card"} w-full px-2 py-4`}>
                <Text
                    className={`${appTheme
                        ? "text-light-standard-text"
                        : "text-dark-standard-text"}`}>Getting reading spots ...</Text>
            </GeneralCard>
        }
    ...
    const LoadInMarkers = () => {
        ...
        return locations.map(
            (spot, key) => <ReadingMarker key={key} location={spot} onMarkerPress={handleSpotPress}/>
        )
    }
    ...
    const getReadingLocations = async () => {
        try {
            const response = await fetch(
                "https://adriaangiel.github.io/data/mocklocations.json"
            );
    ...
    const ConnectionMapView = () => {
        ...
        if(!connection){
            return <View className="w-full h-[300px] bg-gray-500 flex justify-center items-center">
                <Text className="text-2xl">Map is not functional offline</Text>
                </View>
        }

        return  <MapView loadingEnabled={true} userInterfaceStyle="dark" zoom={10} ref={mapRef} 
            style={{
                        width: "100%",
                        height: "300"
                    }} showsCompass={true} zoomEnabled={true} initialRegion={rotterdam} showsUserLocation={true} userLocationPriority={"low"}
                    // onUserLocationChange={(e) => setInitSpotLocation(e.nativeEvent.coordinate)}
                    customMapStyle={appTheme
                        ? lightMapStyle
                        : mapStyle}>
        ...
        <IsLoading state={!location}>
            ...
            <View
                ...
                <View className={`w-full flex gap-2 `}>
                    <Text
                        className={`text-2xl p-2 py-4 ${getColors.getThemeString("bg-dark-card", appTheme)} ${getColors.getThemeString("color-dark-neutral-text", appTheme)}`}>Reading spots near you</Text>
                </View>

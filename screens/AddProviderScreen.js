...
export default function AddProviderScreen() {
  ...
  const ShowMessage = () => {
    if (showSuccess) {
      return (
        <GeneralCard
          custom={"w-[95%] w-min-[100px] gap-2 bg-dark-greon p-5 space-between flex-row"}>
          <View className="px-2">
  ...
  const ScreenBody = () => {
    ...
    return (
      <View className="flex flex-1 gap-6 ">
        <View>
          <Text
            className={`text-5xl ${getColors.getThemeString("text-dark-standard-text", appTheme)}`}>
            Connect a
          </Text>
          <Text
            className={`text-5xl ${getColors.getThemeString("text-dark-active-text", appTheme)}`}>
            Source
          </Text>
        </View>
        <View>
          <Text
            className={`text-xl ${getColors.getThemeString("text-dark-neutral-text", appTheme)}`}>
        ...
        <View className="gap-4 py-6">
          <Text
            className={`text-3xl font-bold ${getColors.getThemeString("text-dark-standard-text", appTheme)}`}>
            Existing providers
          </Text>
  ...
  return (
    <AutocompleteDropdownContextProvider>
      <ScrollView
        className={`px-5 py-2  ${getColors.getThemeString("bg-dark-background", appTheme)}`}>
        <ScreenBody />
      </ScrollView>
    </AutocompleteDropdownContextProvider>
  );
